...
class GameSaveMapper:
    ...
    def from_dict(data: dict) -> GameSaveDTO:
        ...
        return GameSaveDTO(player=player, items=items, points=data.get("points", 0), time=data.get("time", 0.0))
    ...
    def save(session: GameSession) -> None:
        SaverLoader.save(SAVE_FILE, GameSaveMapper.to_dict(GameSaveMapper.to_dto(session)))
